...
class Messages
{
  public:
    ...
    void setReceiverIdToNull() noexcept;

    /**  For column content  */
    ///Get the value of the column content, returns the default value if the column is null
    const std::vector<char> &getValueOfContent() const noexcept;
    ///Return the column value by std::string with binary data
    std::string getValueOfContentAsString() const noexcept;
    ///Return a shared_ptr object pointing to the column const value, or an empty shared_ptr object if the column is null
    const std::shared_ptr<std::vector<char>> &getContent() const noexcept;
    ///Set the value of the column content
    void setContent(const std::vector<char> &pContent) noexcept;
    ...
    Users getSender(const drogon::orm::DbClientPtr &clientPtr) const;
    void getSender(const drogon::orm::DbClientPtr &clientPtr,
                   const std::function<void(Users)> &rcb,
                   const drogon::orm::ExceptionCallback &ecb) const;
    Users getReceiver(const drogon::orm::DbClientPtr &clientPtr) const;
    void getReceiver(const drogon::orm::DbClientPtr &clientPtr,
                     const std::function<void(Users)> &rcb,
                     const drogon::orm::ExceptionCallback &ecb) const;
  private:
